...
class SelectorBIC(ModelSelector):
    ...
    def bic_score(self, n: int):
        model = self.base_model(n) # a GaussianHMM with n states
        f = model.n_features # number of features

        log_l = model.score(self.X, self.lengths) # score for the trained GaussianHMM with c components - log(P(X(c))
        log_n = math.log(len(self.sequences))  # the log of the number of data points
        p = n ** 2 + 2 * n * f - 1 # penalise complexity
...
class SelectorCV(ModelSelector):
    ...
    def cv_model(self, c:int, X:list, lengths:list):
        # with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=DeprecationWarning)
        # warnings.filterwarnings("ignore", category=RuntimeWarning)
        try:
            hmm_model = GaussianHMM(n_components=c, covariance_type="diag", n_iter=1000,
                                    random_state=self.random_state, verbose=False).fit(X, lengths)
            if self.verbose:
                print("model created for {} with {} states".format(self.this_word, c))
            return hmm_model
        except:
            if self.verbose:
                print("failure on {} with {} states".format(self.this_word, c))
        return None
